...
void countingSort(vector<int> &item, vector<int> &novoArray){
    ...
    //Se algum número negativo foi encontrado, então ele será multiplicado por -1 para incrementar o deslocamento dos índices
    if (menorNum < 0)
        menorNum = menorNum * -1;
    ...
    //Realiza soma dos elementos anteriores no array auxiliar, para que saibamos a quantidade de elementos existentes antes de cada valor indica
...
int main(){
    ...
    countingSort(item, vectorOrdenado); //Passa array a ser ordenado e array de saída que armazenará os valores ordenados como parâmetro
